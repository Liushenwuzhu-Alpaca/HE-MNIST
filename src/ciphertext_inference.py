...
class CiphertextInference:
    ...
    def __init__(
        self, context: ts.Context, weights: dict, weight_scale: float = 0.0000001
    ):
        """
        初始化密文推理引擎
        ...
        Args:
            ...
            weights: 模型权重字典
            weight_scale: 权重缩放因子，用于防止密文scale溢出
        """
        # 确保auto_rescale已启用
    ...
    def _load_weights(self):
        """加载并缩放模型权重"""
        # 缩放权重以防止密文乘法后scale溢出
        self.w1 = self.weights["fc1.weight"].numpy() * self.weight_scale
        self.b1 = self.weights["fc1.bias"].numpy() * self.weight_scale
        self.w2 = self.weights["fc2.weight"].numpy() * self.weight_scale
        self.b2 = self.weights["fc2.bias"].numpy() * self.weight_scale
        self.w3 = self.weights["fc3.weight"].numpy() * self.weight_scale
        self.b3 = self.weights["fc3.bias"].numpy() * self.weight_scale
...
class PlainInference:
    ...
    def __init__(self, weights: dict, weight_scale: float = 0.00001):
        self.weights = weights
        self.weight_scale = weight_scale
        self._load_weights()
...
def compare_inference(context_path: str, weights_path: str, test_samples: int = 10):
    ...
    for i in range(test_samples):
        ...
        print(
            f"样本 {i + 1}: 明文预测={plain_pred}, 密文预测={enc_pred}, 一致={plain_correct}, 最大误差={diff:.6f}"
        )
